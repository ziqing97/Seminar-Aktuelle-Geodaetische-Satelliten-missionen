...
xlabel('Time (Day)')
ylabel('Height [km]')
title('a-R = 400 km, e=0, A/m=0.004 m^2/kg as start value');
set(gca,'FontSize',20)

files = dir('H:\space_debris_orbit_integration\orbit_e0_middle');
countlist = zeros(length(files)-2,1);
for i=1:length(files)
    if files(i).isdir
        continue
...
xlabel('Time (Day)')
ylabel('Height [km]')
title('a-R = 600 km, e=0, A/m=0.004 m^2/kg as start value');
set(gca,'FontSize',20)

files = dir('H:\space_debris_orbit_integration\orbit_e0_high');
countlist = zeros(length(files)-2,1);
for i=1:length(files)
    if files(i).isdir
        continue
...
xlabel('Time (Day)')
ylabel('Height [km]')
title('a-R = 1000 km, e=0, A/m=0.004 m^2/kg as start value');
set(gca,'FontSize',20)

sgt = sgtitle('Orbit integration for circular orbit');
sgt.FontSize = 24;